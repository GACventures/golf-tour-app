...
function normalizeScoreEntryLayout(raw: any): ScoreEntryLayout {
...
export default function TourPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      for (const r of normalizedTP) {
        ...
        const effective =
          tourVal !== null && tourVal !== undefined ? Number(tourVal) : globalVal !== null && globalVal !== undefined ? Number(globalVal) : 0;
  ...
  async function saveScoreEntryLayout(next: ScoreEntryLayout) {
    ...
    try {
      const { error: upErr } = await supabase.from("tours").update({ score_entry_layout: next }).eq("id", tourId);
      if (upErr) throw new Error(upErr.message);

      setScoreEntryLayout(next);
      setTour((prev) => (prev ? { ...prev, score_entry_layout: next } : prev));
      setLayoutMsg("Saved ✓");
    } catch (e: any) {
      setLayoutMsg(e?.message ?? "Failed to save layout.");
    } finally {
          ...
          <div>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13 }}>
                <input
                  type="radio"
                  name="score_entry_layout"
                  value="classic"
                  checked={scoreEntryLayout === "classic"}
                  disabled={layoutSaving}
                  onChange={() => void saveScoreEntryLayout("classic")}
                />
                Classic
              </label>
              ...
              <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13 }}>
                <input
                  ...
                  name="score_entry_layout"
                  value="alt"
                  checked={scoreEntryLayout === "alt"}
                  disabled={layoutSaving}
                  onChange={() => void saveScoreEntryLayout("alt")}
                />
                Alt
              </label>
