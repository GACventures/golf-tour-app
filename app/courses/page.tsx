...
const [parsStatus, setParsStatus] = useState('');
...
async function loadPars(courseId: string) {
  ...
  // If nothing in DB yet, initialise all 18 with defaults (including SI)
  if (!data || data.length === 0) {
    const initial: ParRow[] = Array.from({ length: 18 }, (_, i) => ({
      course_id: courseId,
      hole_number: i + 1,
      par: 4,
      stroke_index: i + 1, // default SI
    }));
  ...
  // - ensure stroke_index is always set (fallback = hole_number)
  ...
  for (const row of data as any[]) {
    ...
    byHole.set(hole, {
      ...
      hole_number: hole,
      par: Number.isFinite(Number(row.par)) ? Number(row.par) : 4,
      stroke_index:
        Number.isFinite(Number(row.stroke_index)) && Number(row.stroke_index) >= 1
          ? Number(row.stroke_index)
          : hole, // fallback if missing/null/0
    });
  }
...
async function savePars(selectedCourseId: string) {
  ...
  const payload: ParRow[] = Array.from({ length: 18 }, (_, i) => {
    ...
    const siRaw = existing?.stroke_index;
    ...
    return {
      ...
      par,
      stroke_index:
        Number.isFinite(Number(siRaw)) && Number(siRaw) >= 1
          ? Number(siRaw)
          : hole, // fallback
    };
  });
      ...
      <td>
        <select
          ...
          onChange={(e) => {
            ...
            next[idx] = {
              ...next[idx],
              stroke_index: Number(e.target.value),
            };
            setPars(next);
            setParsStatus('Unsaved changes');
          }}
