...
export default function MobilePlayerSelectPage() {
  ...
  async function load() {
    ...
    setPlayingPlayers(list);

    // If we already had selections and they no longer exist, clear them
    setMeId((prev) => (prev && list.some(p => p.id === prev) ? prev : null));
    setBuddyId((prev) => (prev && list.some(p => p.id === prev) ? prev : null));
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 14, opacity: 0.85 }}>
            <div><strong>Round:</strong> {round?.name ?? '—'}</div>
            {round?.courses?.name && <div><strong>Course:</strong> {round.courses.name}</div>}
          </div>
        </div>
      ...
      {playingPlayers.length === 0 ? (
        <div style={{ marginTop: 16, padding: 12, border: '1px solid #ddd', borderRadius: 8 }}>
          ...
          <p style={{ marginBottom: 0 }}>
            This mobile mode only allows selecting players who are marked as <strong>playing</strong>.
            Go back to the round page and assign players / tick “playing”, then return here.
          </p>
        </div>
      ) : (
        <>
            ...
            <button
              ...
              onClick={() => {
                ...
                try {
                  ...
                  // navigation happens immediately; keep it simple
